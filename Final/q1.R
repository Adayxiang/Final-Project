...
trainData$x_2015=log(trainData$x_2015)
testData$x_2015=log(testData$x_2015)
ytrain = trainData[,12]
ytest = testData[,12]
data$x_2015=log(data$x_2015)
...
test_pre=exp(predict(lm.fit,testData))
(mean((test_pre -  exp(ytest))^2))^(1/2)
mean(abs(test_pre-exp(ytest))/exp(ytest))
...
mean(((exp(ridge.pred)-exp(yR))^2)) ^ (1/2) 
mean(abs(exp(ridge.pred)-exp(yR))/exp(yR))
...
pred=predict(fit,newdata=testData,se = T)
predValue = pred$fit
(mean((exp(pred$fit)-exp(ytest))^2))^0.5
mean(abs(exp(pred$fit)-exp(ytest))/exp(ytest))
"
...
pr.nnRaw <- pr.nn$net.result*(maxs[12]-mins[12])+mins[12]
(sum((exp(pr.nnRaw[,1])-exp(testData['x_2015']))^2)/6260)^0.5
sum(abs(exp(pr.nnRaw)-exp(testData['x_2015'])/exp(testData['x_2015'])))/6260
sum(abs(exp(pred$fit)-exp(ytest))/exp(ytest))/6260



#random forest
library(randomForest)
trainData = data[train,c(-1,-14,-15,-24,-29)]
testData = data[test,c(-1,-14,-15,-24,-29)]
set.seed(1)
...
plot(diff)
(mean((exp(yhat.random)-exp(testData$x_2015))^2))^(1/2)
mean(abs(exp(yhat.random)-exp(testData$x_2015))/exp(testData$x_2015))
...
(mean((exp(yhat.boost)-exp(testData$x_2015))^2))^(1/2)
mean(abs(exp(yhat.boost)-exp(testData$x_2015))/exp(testData$x_2015))
